...
func LoadConfig(path string) (Config, error) {
	cfg := Config{
		ServiceID:            "M18-Cache-State-Management",
		Version:              "0.1.0",
		HTTPPort:             8080,
		GRPCPort:             9090,
	...
	if raw, err := os.ReadFile(path); err == nil {
		...
		if f.Runtime.DefaultTTLSeconds > 0 {
			cfg.DefaultTTL = time.Duration(f.Runtime.DefaultTTLSeconds) * time.Second
		}
	...
	cfg.DefaultTTL = time.Duration(envInt("DEFAULT_TTL_SECONDS", int(cfg.DefaultTTL.Seconds()))) * time.Second
