...
func (c *MemoryConsumer) Receive(context.Context) (*contracts.EventEnvelope, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	if len(c.events) == 0 {
		return nil, io.EOF
...
func (p *LoggingDLQPublisher) PublishDLQ(context.Context, contracts.DLQRecord) error { return nil }
