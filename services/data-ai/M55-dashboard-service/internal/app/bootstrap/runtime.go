...
import (
	...
	eventadapter "github.com/viralforge/mesh/services/data-ai/M55-dashboard-service/internal/adapters/events"
	grpcadapter "github.com/viralforge/mesh/services/data-ai/M55-dashboard-service/internal/adapters/grpc"
	httpadapter "github.com/viralforge/mesh/services/data-ai/M55-dashboard-service/internal/adapters/http"
...
func NewRuntime(ctx context.Context, configPath string) (*Runtime, error) {
	...
	service := application.NewService(application.Dependencies{
		...
		Outbox:        repos.Outbox,
		Profile:       grpcadapter.NewProfileClient(cfg.ProfileGRPCURL),
		Billing:       grpcadapter.NewBillingClient(cfg.BillingGRPCURL),
		Content:       grpcadapter.NewContentClient(cfg.ContentGRPCURL),
		Escrow:        grpcadapter.NewEscrowClient(cfg.EscrowGRPCURL),
		Onboarding:    grpcadapter.NewOnboardingClient(cfg.OnboardingGRPCURL),
		Finance:       grpcadapter.NewFinanceClient(cfg.FinanceGRPCURL),
		Rewards:       grpcadapter.NewRewardClient(cfg.RewardGRPCURL),
		Gamification:  grpcadapter.NewGamificationClient(cfg.GamificationGRPCURL),
		Analytics:     grpcadapter.NewAnalyticsClient(cfg.AnalyticsGRPCURL),
		Products:      grpcadapter.NewProductClient(cfg.ProductGRPCURL),
		DLQ:           dlqPublisher,
	})
	...
	grpcadapter.Register(grpcServer, grpcadapter.NewDashboardInternalServer(service))
