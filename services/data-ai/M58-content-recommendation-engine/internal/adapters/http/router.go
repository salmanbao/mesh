...
func NewRouter(handler *Handler) http.Handler {
	...
	r.Route("/api/v1", func(r chi.Router) {
		r.Group(func(r chi.Router) {
			...
			r.Post("/recommendations/{recommendation_id}/feedback", handler.recordFeedback)
			r.Route("/admin", func(r chi.Router) {
				r.Post("/recommendation-overrides", handler.createOverride)
				r.Get("/recommendation-overrides", handler.listOverrides)
			})
		})
	})
	return r
