...
type Config struct {
	...
	CampaignDiscoveryGRPCURL string
...
type configFile struct {
	...
	Dependencies struct {
		CampaignDiscoveryGRPCURL string `yaml:"campaign_discovery_grpc_url"`
	} `yaml:"dependencies"`
}

func LoadConfig(path string) (Config, error) {
	cfg := Config{
		ServiceID:            "M58-Content-Recommendation-Engine",
		HTTPPort:             8080,
		GRPCPort:             9090,
	...
	if raw, err := os.ReadFile(path); err == nil {
		...
		cfg.CampaignDiscoveryGRPCURL = f.Dependencies.CampaignDiscoveryGRPCURL
	}

	cfg.CampaignDiscoveryGRPCURL = envOrDefault("CAMPAIGN_DISCOVERY_GRPC_URL", cfg.CampaignDiscoveryGRPCURL)
