package application

import (
	"context"
	"encoding/json"
	"fmt"
	"strings"

	"github.com/google/uuid"
	"github.com/viralforge/mesh/services/data-ai/M58-content-recommendation-engine/internal/contracts"
	"github.com/viralforge/mesh/services/data-ai/M58-content-recommendation-engine/internal/domain"
	"github.com/viralforge/mesh/services/data-ai/M58-content-recommendation-engine/internal/ports"
)

func (s *Service) HandleInboundEvent(_ context.Context, event contracts.EventEnvelope) error {
	if err := validateEventEnvelope(event); err != nil {
		return err
	}
	// Canonical dependencies currently declare no EVENT deps for M58.
	return domain.ErrUnsupportedEventType
}

func (s *Service) FlushOutbox(ctx context.Context) error {
	if s.outbox == nil {
		return nil
	}
	pending, err := s.outbox.ListPending(ctx, s.cfg.OutboxFlushBatchSize)
	if err != nil {
		return err
	}
	for _, record := range pending {
		now := s.nowFn()
		switch record.EventClass {
		case domain.CanonicalEventClassDomain:
			if s.eventDedup != nil {
				dup, err := s.eventDedup.IsDuplicate(ctx, record.Envelope.EventID, now)
				if err != nil {
					return err
				}
				if dup {
					if err := s.outbox.MarkSent(ctx, record.RecordID, now); err != nil {
						return err
					}
					continue
				}
			}
			if s.domainEvents != nil {
				if err := s.domainEvents.PublishDomain(ctx, record.Envelope); err != nil {
					// Domain events require DLQ on publish failure.
					if s.dlq != nil {
						nowDLQ := s.nowFn()
						_ = s.dlq.PublishDLQ(ctx, contracts.DLQRecord{
							OriginalEvent: record.Envelope,
							ErrorSummary:  err.Error(),
							RetryCount:    1,
							FirstSeenAt:   nowDLQ,
							LastErrorAt:   nowDLQ,
							SourceTopic:   record.Envelope.EventType,
							DLQTopic:      "recommendation-service.dlq",
							TraceID:       record.Envelope.TraceID,
						})
					}
					return err
				}
			}
			if s.eventDedup != nil {
				if err := s.eventDedup.MarkProcessed(ctx, record.Envelope.EventID, record.Envelope.EventType, now.Add(s.cfg.EventDedupTTL)); err != nil {
					return err
				}
			}
		case domain.CanonicalEventClassAnalyticsOnly:
			if s.analytics != nil {
				if err := s.analytics.PublishAnalytics(ctx, record.Envelope); err != nil {
					return err
				}
			}
		default:
			return fmt.Errorf("%w: %s", domain.ErrUnsupportedEventClass, record.EventClass)
		}
		if err := s.outbox.MarkSent(ctx, record.RecordID, now); err != nil {
			return err
		}
	}
	return nil
}

func (s *Service) enqueueRecommendationGenerated(ctx context.Context, actor Actor, batch domain.RecommendationBatch, first domain.Recommendation) error {
	payload := contracts.RecommendationGeneratedPayload{
		RecommendationID:      first.RecommendationID,
		RecommendationBatchID: batch.BatchID,
		EntityID:              first.EntityID,
		ModelVersion:          batch.ModelVersion,
		RecommendationScore:   first.Score,
		UserID:                batch.UserID,
	}
	return s.enqueueDomainEvent(ctx, domain.EventRecommendationGenerated, domain.CanonicalPartitionKeyPath(domain.EventRecommendationGenerated), batch.UserID, actor.RequestID, payload)
}

func (s *Service) enqueueRecommendationFeedbackRecorded(ctx context.Context, actor Actor, feedback domain.FeedbackRecord) error {
	payload := contracts.RecommendationFeedbackRecordedPayload{
		FeedbackID:        feedback.FeedbackID,
		RecommendationID:  feedback.RecommendationID,
		EntityID:          feedback.EntityID,
		FeedbackEventType: feedback.EventType,
		UserID:            feedback.UserID,
	}
	return s.enqueueDomainEvent(ctx, domain.EventRecommendationFeedbackRecorded, domain.CanonicalPartitionKeyPath(domain.EventRecommendationFeedbackRecorded), feedback.UserID, actor.RequestID, payload)
}

func (s *Service) enqueueRecommendationOverrideApplied(ctx context.Context, actor Actor, ov domain.RecommendationOverride) error {
	payload := contracts.RecommendationOverrideAppliedPayload{
		OverrideID:    ov.OverrideID,
		EntityID:      ov.EntityID,
		Multiplier:    ov.Multiplier,
		AffectedUsers: 0,
		OverrideType:  ov.OverrideType,
	}
	return s.enqueueDomainEvent(ctx, domain.EventRecommendationOverrideApplied, domain.CanonicalPartitionKeyPath(domain.EventRecommendationOverrideApplied), ov.OverrideID, actor.RequestID, payload)
}

func (s *Service) enqueueDomainEvent(ctx context.Context, eventType, partitionPath, partitionKey, traceID string, payload interface{}) error {
	if s.outbox == nil {
		return nil
	}
	data, err := json.Marshal(payload)
	if err != nil {
		return err
	}
	envelope := contracts.EventEnvelope{
		EventID:          uuid.NewString(),
		EventType:        eventType,
		EventClass:       domain.CanonicalEventClassDomain,
		OccurredAt:       s.nowFn(),
		PartitionKeyPath: partitionPath,
		PartitionKey:     partitionKey,
		SourceService:    s.cfg.ServiceName,
		TraceID:          nonEmpty(traceID, uuid.NewString()),
		SchemaVersion:    "1.0",
		Data:             data,
	}
	if err := validatePartitionKeyInvariant(envelope, partitionPath); err != nil {
		return err
	}
	return s.outbox.Enqueue(ctx, ports.OutboxRecord{RecordID: uuid.NewString(), EventClass: domain.CanonicalEventClassDomain, Envelope: envelope, CreatedAt: s.nowFn()})
}

func (s *Service) publishDLQIdempotencyConflict(ctx context.Context, key, traceID string) error {
	if s.dlq == nil {
		return nil
	}
	now := s.nowFn()
	return s.dlq.PublishDLQ(ctx, contracts.DLQRecord{
		OriginalEvent: contracts.EventEnvelope{
			EventID:          uuid.NewString(),
			EventType:        "recommendation.idempotency.conflict",
			EventClass:       domain.CanonicalEventClassOps,
			OccurredAt:       now,
			PartitionKeyPath: "envelope.source_service",
			PartitionKey:     s.cfg.ServiceName,
			SourceService:    s.cfg.ServiceName,
			TraceID:          nonEmpty(traceID, uuid.NewString()),
			SchemaVersion:    "1.0",
			Data:             []byte(`{"source":"api","key":"` + key + `"}`),
		},
		ErrorSummary: "idempotency key reused with mismatched payload",
		RetryCount:   1,
		FirstSeenAt:  now,
		LastErrorAt:  now,
		SourceTopic:  "api",
		DLQTopic:     "recommendation-service.dlq",
		TraceID:      nonEmpty(traceID, uuid.NewString()),
	})
}

func validateEventEnvelope(event contracts.EventEnvelope) error {
	if strings.TrimSpace(event.EventID) == "" || strings.TrimSpace(event.EventType) == "" || event.OccurredAt.IsZero() {
		return domain.ErrInvalidEnvelope
	}
	if strings.TrimSpace(event.SourceService) == "" || strings.TrimSpace(event.TraceID) == "" || strings.TrimSpace(event.SchemaVersion) == "" {
		return domain.ErrInvalidEnvelope
	}
	if len(event.Data) == 0 {
		return domain.ErrInvalidEnvelope
	}
	return nil
}

func validatePartitionKeyInvariant(event contracts.EventEnvelope, expectedPath string) error {
	if strings.TrimSpace(expectedPath) == "" || event.PartitionKeyPath != expectedPath {
		return domain.ErrInvalidEnvelope
	}
	if strings.HasPrefix(expectedPath, "data.") {
		field := strings.TrimPrefix(expectedPath, "data.")
		var payload map[string]interface{}
		if err := json.Unmarshal(event.Data, &payload); err != nil {
			return domain.ErrInvalidEnvelope
		}
		value, ok := payload[field]
		if !ok || fmt.Sprint(value) != event.PartitionKey {
			return domain.ErrInvalidEnvelope
		}
		return nil
	}
	if expectedPath == "envelope.source_service" {
		if event.PartitionKey != event.SourceService {
			return domain.ErrInvalidEnvelope
		}
		return nil
	}
	return domain.ErrInvalidEnvelope
}

func nonEmpty(v, fallback string) string {
	if strings.TrimSpace(v) != "" {
		return strings.TrimSpace(v)
	}
	return fallback
}
