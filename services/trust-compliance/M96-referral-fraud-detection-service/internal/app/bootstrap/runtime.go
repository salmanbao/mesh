...
import (
	...
	httpadapter "github.com/viralforge/mesh/services/trust-compliance/M96-referral-fraud-detection-service/internal/adapters/http"
...
func NewRuntime(_ context.Context, configPath string) (*Runtime, error) {
	...
	svc := application.NewService(application.Dependencies{
		Config:         application.Config{ServiceName: cfg.ServiceID, IdempotencyTTL: cfg.IdempotencyTTL, EventDedupTTL: cfg.EventDedupTTL, OutboxFlushBatchSize: cfg.OutboxFlushBatchSize, DefaultThreshold: cfg.DefaultThreshold},
		ReferralEvents: repos.ReferralEvents, Decisions: repos.Decisions, Policies: repos.Policies, Fingerprints: repos.Fingerprints, Clusters: repos.Clusters, Disputes: repos.Disputes, AuditLogs: repos.AuditLogs, Idempotency: repos.Idempotency, EventDedup: repos.EventDedup, Outbox: repos.Outbox,
		Affiliate: grpcadapter.NewAffiliateClient(cfg.AffiliateGRPCURL), DomainEvents: domainPub, Analytics: analyticsPub, DLQ: dlqPub,
	})
	...
	grpcadapter.Register(grpcServer, grpcadapter.NewReferralFraudInternalServer(svc))
