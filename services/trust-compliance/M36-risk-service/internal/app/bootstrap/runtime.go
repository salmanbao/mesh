...
import (
	...
	httpadapter "github.com/viralforge/mesh/services/trust-compliance/M36-risk-service/internal/adapters/http"
...
func NewRuntime(_ context.Context, configPath string) (*Runtime, error) {
	...
	svc := application.NewService(application.Dependencies{
		...
		Fraud:        grpcadapter.NewFraudClient(cfg.FraudGRPCURL),
		Moderation:   grpcadapter.NewModerationClient(cfg.ModerationGRPCURL),
		Resolution:   grpcadapter.NewResolutionClient(cfg.ResolutionGRPCURL),
		Reputation:   grpcadapter.NewReputationClient(cfg.ReputationGRPCURL),
	...
	grpcadapter.Register(grpcServer, grpcadapter.NewRiskInternalServer(svc))
