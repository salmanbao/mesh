...
type PostValidationRepository interface {
	Create(ctx context.Context, row domain.PostValidation) error
	UpsertByUserPlatformPost(ctx context.Context, row domain.PostValidation) error
	GetByUserPlatformPost(ctx context.Context, userID, platform, postID string) (domain.PostValidation, error)
}

type SocialMetricRepository interface {
	Append(ctx context.Context, row domain.SocialMetric) error
	LatestByUserPlatform(ctx context.Context, userID, platform string) (domain.SocialMetric, error)
}
